...
const UserUpdate = ({ userId, fetchUsers, closeModal }) => {
  ...
  useEffect(() => {
    ...
    fetchUserData(); // Llamar a la función para cargar los datos del usuario al cargar el componente
  }, []);
  ...
  const fetchUserData = async () => {
    try {
      ...
      const roleIds = userData.roles.map(role => role._id);
      console.log(roleIds)
      ...
      setFormData({
        ...
        role: roleIds // Asignar todos los IDs de roles
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchUsers(); // Actualizar la lista de usuarios después de modificar uno
      closeModal();
      Swal.fire({
        icon: 'success',
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold mb-4 text-center">Actualizar Usuario</h2>
      <form onSubmit={handleSubmit}>
        ...
        */}
        ...
        <div className="mb-6">
          <button type="submit" className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-md focus:outline-none focus:ring focus:ring-blue-200">Actualizar Usuario</button>
        </div>
      </form>
      <div className="mb-4">
